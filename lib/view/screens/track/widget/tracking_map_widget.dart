...
class _TrackingMapWidgetState extends State<TrackingMapWidget> {
  ...
  void initState() {
    super.initState();
    print('====lat :${widget.deliveryManModel.latitude}');
    print('====lat :${Provider.of<SplashProvider>(context, listen: false).configModel.restaurantLocationCoverage}');

    RestaurantLocationCoverage coverage =
        Provider.of<SplashProvider>(context, listen: false).configModel.restaurantLocationCoverage;
    _deliveryBoyLatLng = LatLng(
        double.parse(widget.deliveryManModel.latitude ?? '0'), double.parse(widget.deliveryManModel.longitude ?? '0'));
    _addressLatLng = widget.addressModel != null
        ? LatLng(double.parse(widget.addressModel.latitude), double.parse(widget.addressModel.longitude))
        : LatLng(0, 0);
    _restaurantLatLng = LatLng(double.parse(coverage.latitude), double.parse(coverage.longitude));
    print('====address target :${_addressLatLng}');
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      height: 200,
      width: ResponsiveHelper.isMobilePhone() ? _width - 130 : 1170.0 - 100.0,
  ...
  void setMarker() async {
    try {
      BitmapDescriptor _restaurantImageData;
      BitmapDescriptor _deliveryBoyImageData;
      BitmapDescriptor _destinationImageData;

      await BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(30, 50)), Images.restaurant_marker)
          .then((_marker) {
        _restaurantImageData = _marker;
      });

      await BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(30, 50)), Images.delivery_boy_marker)
          .then((_marker) {
        _deliveryBoyImageData = _marker;
      });

      await BitmapDescriptor.fromAssetImage(ImageConfiguration(size: Size(30, 50)), Images.destination_marker)
          .then((_marker) {
        _destinationImageData = _marker;
      });
