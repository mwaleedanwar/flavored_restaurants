...
class _TrackingMapWidgetState extends State<TrackingMapWidget> {
  ...
  void initState() {
    super.initState();
    debugPrint('====lat :${widget.deliveryManModel?.latitude}');
    debugPrint(
        '====lat :${Provider.of<SplashProvider>(context, listen: false).configModel!.restaurantLocationCoverage}');
    ...
    _deliveryBoyLatLng = LatLng(double.parse(widget.deliveryManModel?.latitude ?? '0'),
  ...
  void setMarker() async {
    try {
      ...
      BitmapDescriptor deliveryBoyImageData =
          await BitmapDescriptor.asset(const ImageConfiguration(size: Size(30, 50)), Images.delivery_boy_marker);

      BitmapDescriptor destinationImageData =
          await BitmapDescriptor.asset(const ImageConfiguration(size: Size(30, 50)), Images.destination_marker);
      LatLngBounds bounds;
      double rotation = 0;
      if (_controller != null) {
      ...
      debugPrint('ERROR TRACKING MAP WIDGET $e');
    }
