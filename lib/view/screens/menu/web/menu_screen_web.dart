...
class MenuScreenWeb extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<MenuModel> menuList = [
      ...
      if (splashProvider.policyModel?.refundPage != null && splashProvider.policyModel!.refundPage!.status)
        MenuModel(
            icon: Images.refundPolicy,
            title: getTranslated('refund_policy', context),
            route: Routes.getRefundPolicyRoute()),

      if (splashProvider.policyModel?.returnPage != null && splashProvider.policyModel!.returnPage!.status)
        MenuModel(
            icon: Images.returnPolicy,
            title: getTranslated('return_policy', context),
            route: Routes.getReturnPolicyRoute()),

      if (splashProvider.policyModel?.cancellationPage != null && splashProvider.policyModel!.cancellationPage!.status)
        MenuModel(
                      ...
                      child: Builder(builder: (context) {
                        return Container(
                          ...
                          decoration: BoxDecoration(
                              ...
                              boxShadow: [
                                BoxShadow(
                                    color: Colors.white.withOpacity(0.1), blurRadius: 22, offset: const Offset(0, 8.8))
                              ]),
                          child: ClipOval(
                            child: isLoggedIn
                                ? ImageWidget(
