...
class ProfileRepo {
  ...
  getAddressTypeList() async {
    try {
      ...
      return addressTypeList; //ApiResponse.withSuccess(response);
    } catch (e) {
  ...
  Future<http.StreamedResponse> updateProfile(
      UserInfoModel userInfoModel, String password, File file, XFile data, String token) async {
    http.MultipartRequest request =
        http.MultipartRequest('POST', Uri.parse(F.BASE_URL + AppConstants.UPDATE_PROFILE_URI));
    ...
    if (file != null) {
      ...
      var part = http.MultipartFile('image', data.readAsBytes().asStream(), _list.length,
          filename: basename(data.path), contentType: MediaType('image', 'jpg'));
