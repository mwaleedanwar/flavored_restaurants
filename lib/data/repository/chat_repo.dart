...
class ChatRepo {
  ...
  Future<ApiResponse> getDeliveryManMessage(int orderId, int offset) async {
    try {
      final response = await httpClient
          .get('${AppConstants.GET_DELIVERYMAN_MESSAGE_URI}?offset=$offset&limit=100&order_id=$orderId');
  ...
  Future<ApiResponse> getAdminMessage(int offset) async {
    ...
    try {
      final response = await httpClient.get('${AppConstants.GET_ADMIN_MESSAGE_URL}?offset=$offset&limit=100');
  ...
  Future<http.StreamedResponse> sendMessageToDeliveryMan(
    ...
    http.MultipartRequest request =
        http.MultipartRequest('POST', Uri.parse('${F.BASE_URL}${AppConstants.SEND_MESSAGE_TO_DELIVERY_MAN_URL}'));
    request.headers.addAll(<String, String>{'Authorization': 'Bearer $token'});
    for (int i = 0; i < file.length; i++) {
      if (file != null) {
        Uint8List _list = await file[i].readAsBytes();
        var part = http.MultipartFile('image[]', file[i].readAsBytes().asStream(), _list.length,
            filename: basename(file[i].path), contentType: MediaType('image', 'jpg'));
  ...
  Future<http.StreamedResponse> sendMessageToAdmin(String message, List<XFile> file, String token) async {
    ...
    debugPrint('---message body;message:${message},List Files:${file},token: ${token}');

    http.MultipartRequest request =
        http.MultipartRequest('POST', Uri.parse(F.BASE_URL + AppConstants.SEND_MESSAGE_TO_ADMIN_URL));
    request.headers.addAll(<String, String>{'Authorization': 'Bearer $token'});
    for (int i = 0; i < file.length; i++) {
      if (file != null) {
        Uint8List _list = await file[i].readAsBytes();
  ...
  Future<http.StreamedResponse> sendMessage(String message, List<XFile> images, String token) async {
    http.MultipartRequest request = http.MultipartRequest('POST', Uri.parse(F.BASE_URL + AppConstants.GET_IMAGES_URL));
    request.headers.addAll(<String, String>{'Authorization': 'Bearer $token'});
    if (images != null && ResponsiveHelper.isMobilePhone()) {
      for (int i = 0; i < images.length; i++) {
        File _file = File(images[i].path);
        request.files.add(http.MultipartFile('image[]', _file.readAsBytes().asStream(), _file.lengthSync(),
            filename: _file.path.split('/').last));
      }
    } else if (images != null && ResponsiveHelper.isWeb()) {
      for (int i = 0; i < images.length; i++) {
        Uint8List _list = await images[i].readAsBytes();
        request.files.add(http.MultipartFile('image[]', images[i].readAsBytes().asStream(), _list.length,
  ...
  Future<ResponseModel> sendRealTimeMessage(
      int senderID, UserType senderType, int receiverID, UserType receiverType, String message, List<XFile> images,
      {int orderID}) async {
    ...
    http.MultipartRequest request = http.MultipartRequest('POST', Uri.parse(AppConstants.GET_IMAGES_URL));
    request.headers
        .addAll(<String, String>{'Authorization': 'Bearer ${sharedPreferences.getString(AppConstants.TOKEN)}'});
    ...
    await _receiverRef
        .doc(_receiverId)
        .set(_message.toJson())
        .then((value) => _responseModel = ResponseModel(true, 'added'))
        .catchError((error) {
  ...
  Map<String, dynamic> getRealTimeMessages(int senderID, UserType senderType, int receiverID, UserType receiverType,
      {int orderID}) {
    FirebaseFirestore _fireStore = FirebaseFirestore.instance;
    Query _ref;
